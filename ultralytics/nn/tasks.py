...
from ultralytics.nn.extra_modules.block import CoTBlock,DynamicIncMixerBlock
...
def _predict_once(self, x, profile=False, visualize=False, embed=None):
    for i, m in enumerate(self.model):
        if isinstance(m, Concat):
            print(f"Concat at layer {i}:")
            print(f"  Input tensors sizes: {[t.shape for t in x]}")
        x = m(x)  # run
        if isinstance(m, Concat):
            print(f"  Output tensor size: {x.shape}")
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m is IdentityConv:
            # 特殊处理恒等层
            args = [ch[f]] if not args else args  # 使用前层通道作为默认
            if len(args) < 1:
                args.append(ch[f])  # 确保有通道参数
            m_ = m(*args)  # 实例化模块
            c2 = args[0]
        if m in ((
            ...
            if m in SEGMENT_CLASS:
                ...
                if m in (Segment_LSCD, Segment_TADDH, Segment_LSCSBD, Segment_LSDECD, Segment_RSCD, Segment_LSCD_LQE):
                    args[3] = make_divisible(min(args[3], max_channelTEMs) * width, 8)
            ...
            c2 = m.channel
                   ...
                   FSA, AttentiveLayer, EUCB_SC,LearnableColorCorrector,DynamicIncMixerBlock}:
            c2 = ch[f]
            args = [c2, *args]
            # print(args)
        elif m is CoTBlock:
