...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'A2C2f_EDFFN', 'C3k2_EBlock', 'C3k2_DBlock','C3k2_FDConv', 'C3k2_MambaOut_FDConv', 'C3k2_PFDConv', 'C3k2_PFDConv', 'C3k2_FasterFD', 'C3k2_DSAN', 'C3k2_MambaOut_DSA', 'C3k2_DSA', 'C3k2_DSAN_EDFFN', 'C3k2_RMB','LearnableColorCorrector'
           ]
...
class SPDConv(nn.Module):
    ...
    def __init__(self, inc, ouc, dimension=1):
        ...
        # 使用Sequential组合卷积、批归一化和激活函数
        self.conv = nn.Sequential(
            nn.Conv2d(inc * 4, ouc, kernel_size=3, padding=1),  # 卷积层
            nn.BatchNorm2d(ouc),  # 批归一化层[3,6](@ref)
            nn.ReLU(inplace=True)  # 激活函数[9](@ref)
        )
...
######################################## color start ########################################
class LearnableColorCorrector(nn.Module):
    def __init__(self, in_channels, *args, **kwargs):
        super().__init__()
        self.in_channels = in_channels

        # 解析参数
        self.max_iter = args[0] if len(args) > 0 else 8
        self.use_refinement = kwargs.get('use_refinement', True)
        self.refine_channels = kwargs.get('refine_channels', 8)

        # 核心可学习参数
        self.channel_weights = nn.Parameter(torch.tensor([0.6, 0.4], dtype=torch.float32))
        self.gain_factors = nn.Parameter(torch.tensor([0.8, 1.2], dtype=torch.float32))
        self.adapt_coeff = nn.Parameter(torch.tensor(5.0, dtype=torch.float32))

        # 微调模块 - 显式设置为FP32
        if self.use_refinement:
            self.refinement = nn.Sequential(
                nn.Conv2d(3, self.refine_channels, 3, padding=1),
                nn.ReLU(),
                nn.Conv2d(self.refine_channels, 3, 1)
            )
            # 确保微调模块权重初始化在FP32
            for p in self.refinement.parameters():
                p.data = p.data.float()
        else:
        ...
        if self.in_channels != 3:
            raise ValueError(f"LearnableColorCorrector只支持3通道输入，但得到 in_channels={in_channels}")

        # 添加autocast属性以支持AMP（自动混合精度）
        self.autocast = False  # 表示此模块不支持AMP自动转换

    def forward(self, x):
        # 记录输入数据类型以便在结束时恢复
        input_dtype = x.dtype

        # 确保内部计算使用FP32（即使输入是FP16）
        x = x.float()
        ...
        # 预处理：转换为0-1范围
        x_float = x / 255.0
        batch_size, channels, height, width = x_float.shape
        if channels != 3:
            raise ValueError(f"输入必须有3个通道，但得到 {channels}个通道")

        # 分离RGB通道
        r = x_float[:, 0, :, :]
        g = x_float[:, 1, :, :]
        b = x_float[:, 2, :, :]
        all_channels = {'R': r, 'G': g, 'B': b}

        # 计算通道分数
        scores = {}
        for name, channel in all_channels.items():
            mean_val = torch.mean(channel, dim=[1, 2], keepdim=True)
            var_val = torch.var(channel, dim=[1, 2], keepdim=True)
            scores[name] = (self.channel_weights[0] * mean_val +
                            self.channel_weights[1] * (1 - var_val))

        # 创建分数张量
        score_tensor = torch.stack([scores['R'], scores['G'], scores['B']], dim=1)
        score_squeezed = score_tensor.squeeze(-1).squeeze(-1)

        # 找出最大和最小通道索引
        max_idx = torch.argmax(score_squeezed, dim=1)
        min_idx = torch.argmin(score_squeezed, dim=1)

        # 分离通道
        max_channel = torch.zeros_like(r)
        min_channel = torch.zeros_like(r)
        mid_channel = torch.zeros_like(r)

        for i in range(batch_size):
            channels_batch = {'R': r[i], 'G': g[i], 'B': b[i]}

            # 最大通道
            if max_idx[i] == 0:
                max_channel[i] = channels_batch['R']
            elif max_idx[i] == 1:
                max_channel[i] = channels_batch['G']
            else:
                max_channel[i] = channels_batch['B']

            # 最小通道
            if min_idx[i] == 0:
                min_channel[i] = channels_batch['R']
            elif min_idx[i] == 1:
                min_channel[i] = channels_batch['G']
            else:
                min_channel[i] = channels_batch['B']

            # 中间通道
            if (max_idx[i] != 0 and min_idx[i] != 0):
                mid_channel[i] = channels_batch['R']
            elif (max_idx[i] != 1 and min_idx[i] != 1):
                mid_channel[i] = channels_batch['G']
            else:
                mid_channel[i] = channels_batch['B']

        # 自适应增益计算
        max_channel_expanded = max_channel.unsqueeze(1)
        local_var = F.avg_pool2d(max_channel_expanded, 7, padding=3, stride=1)
        gain_factor = 0.5 + 0.5 * torch.exp(-self.adapt_coeff * local_var.squeeze(1))

        # 归一化最大通道
        min_val = torch.amin(max_channel, dim=[1, 2], keepdim=True)
        max_val = torch.amax(max_channel, dim=[1, 2], keepdim=True)
        range_val = max_val - min_val + 1e-7
        max_norm = (max_channel - min_val) / range_val
        max_mean = torch.mean(max_norm, dim=[1, 2], keepdim=True)

        # 迭代调整
        for _ in range(self.max_iter):
            mid_mean = torch.mean(mid_channel, dim=[1, 2], keepdim=True)
            min_mean = torch.mean(min_channel, dim=[1, 2], keepdim=True)

            mid_adjust = (max_mean - mid_mean) * max_norm
            min_adjust = (max_mean - min_mean) * torch.sqrt(max_norm + 0.1)

            mid_channel = torch.clamp(
                mid_channel + mid_adjust * gain_factor * self.gain_factors[0],
                0, 1
            )
            min_channel = torch.clamp(
                min_channel + min_adjust * gain_factor * self.gain_factors[1],
                0, 1
            )

        # 重构图像
        r_corrected = torch.zeros_like(r)
        g_corrected = torch.zeros_like(g)
        b_corrected = torch.zeros_like(b)

        for i in range(batch_size):
            # 最大通道
            if max_idx[i] == 0:
                r_corrected[i] = max_norm[i]
            elif max_idx[i] == 1:
                g_corrected[i] = max_norm[i]
            else:
                b_corrected[i] = max_norm[i]

            # 最小通道
            if min_idx[i] == 0:
                r_corrected[i] = min_channel[i]
            elif min_idx[i] == 1:
                g_corrected[i] = min_channel[i]
            else:
                b_corrected[i] = min_channel[i]

            # 中间通道
            if (max_idx[i] != 0 and min_idx[i] != 0):
                r_corrected[i] = mid_channel[i]
            elif (max_idx[i] != 1 and min_idx[i] != 1):
                g_corrected[i] = mid_channel[i]
            else:
                b_corrected[i] = mid_channel[i]

        # 组合通道
        corrected = torch.stack([r_corrected, g_corrected, b_corrected], dim=1)

        # 应用微调模块
        if self.refinement is not None:
            # 确保微调模块在FP32下
            refinement_module = self.refinement.float()
            for p in refinement_module.parameters():
                p.data = p.data.float()

            refined = refinement_module(corrected)
            corrected = corrected + refined
        ...
        # 关键修改：将输出转换回原始输入类型（通常是FP16）
        # 如果AMP启用，转换回半精度；否则保持FP32
        return corrected.to(input_dtype)
...
class CoTBlock(nn.Module):
    def __init__(self, in_channels, out_channels, groups=4, reduction_ratio=4):
        ...
        self.downsample = nn.Sequential(
            ...
            # 第三次下采样 (stride=2)
            nn.Conv2d(in_channels * 2, in_channels * 4, 3, stride=2, padding=1,
                      groups=in_channels, bias=False),
        ...
        # 注意力机制
        self.attention = nn.Sequential(
            nn.Conv2d(2 * out_channels, out_channels // 2, 1),
    ...
    def forward(self, x):
        # 逐步下采样 [B, C, H, W] -> [B, 4C, H/8, W/8]
        x = self.downsample(x)

        # 通道调整 [B, 4C, H/8, W/8] -> [B, outC, H/8, W/8]
        x = self.channel_adjust(x)

        # 全局上下文信息 [B, outC, H/8, W/8] -> [B, outC, 1, 1]
        context = self.context_branch(x)

        # 特征提取 [B, outC, H/8, W/8] -> [B, outC, H/8, W/8]
        X = self.feature_path(x)

        # 组卷积特征 [B, outC, H/8, W/8] -> [B, outC, H/8, W/8]
        D = self.group_conv(X)

        # 注意力机制
        concat = torch.cat([X, D], dim=1)  # [B, 2*outC, H/8, W/8]
        M = self.attention(concat)  # [B, outC, H/8, W/8]

        # 特征投影与加权
        projected = self.projection(X)  # [B, outC, H/8, W/8]
        F = M * projected  # [B, outC, H/8, W/8]

        # 残差输出（加入上下文信息）
        O = F * context + D  # [B, outC, H/8, W/8]
...
class LaplacianConv(nn.Module):
    def __init__(self, channel, kernel_type='8neighbor', return_abs=True):
        """
        修改后与SobelConv/PrewittConv保持统一接口
        :param channel: 输入通道数
        ...
        # 转换为3D卷积格式 [C, 1, 1, 3, 3]
        laplacian_kernel = torch.tensor(kernel, dtype=torch.float32).unsqueeze(0).expand(channel, 1, 1, 3, 3)

        # 构建3D卷积层 (深度维度为1)
        self.conv3d = nn.Conv3d(
            channel, channel, kernel_size=(1, 3, 3),
            padding=(0, 1, 1), groups=channel, bias=False
        )

        # 设置权重并冻结
        self.conv3d.weight.data = laplacian_kernel.clone()
        self.conv3d.weight.requires_grad_(False)

    def forward(self, x):
        # 添加虚拟深度维度: [B, C, H, W] -> [B, C, 1, H, W]
        x_3d = x.unsqueeze(2)

        # 执行3D卷积
        output = self.conv3d(x_3d)

        # 移除深度维度: [B, C, 1, H, W] -> [B, C, H, W]
...
class PrewittConv(nn.Module):
    def __init__(self, channel) -> None:
        ...
        # 转换为张量并扩展维度以匹配3D卷积
        prewitt_kernel_x = torch.tensor(prewitt_x, dtype=torch.float32).unsqueeze(0).expand(channel, 1, 1, 3, 3)
        prewitt_kernel_y = torch.tensor(prewitt_y, dtype=torch.float32).unsqueeze(0).expand(channel, 1, 1, 3, 3)

        # 创建3D卷积层
        self.prewitt_kernel_x_conv3d = nn.Conv3d(channel, channel, kernel_size=3, padding=1, groups=channel, bias=False)
        self.prewitt_kernel_y_conv3d = nn.Conv3d(channel, channel, kernel_size=3, padding=1, groups=channel, bias=False)

        # 设置卷积核权重
        self.prewitt_kernel_x_conv3d.weight.data = prewitt_kernel_x.clone()
        self.prewitt_kernel_y_conv3d.weight.data = prewitt_kernel_y.clone()

        # 冻结权重
        self.prewitt_kernel_x_conv3d.requires_grad = False
        self.prewitt_kernel_y_conv3d.requires_grad = False

    def forward(self, x):
        # 添加虚拟深度维度 -> [B, C, 1, H, W]
        # 应用3D卷积后移除深度维度 -> [B, C, H, W]
        return (self.prewitt_kernel_x_conv3d(x[:, :, None, :, :]) +
                self.prewitt_kernel_y_conv3d(x[:, :, None, :, :]))[:, :, 0]
...
class DynamicIncMixerBlock(nn.Module):
    def __init__(self, in_dim, out_dim, drop_path=0.0, downsample_mode='conv'):
        """
        in_dim: 输入通道数
        out_dim: 输出通道数（通常是2倍输入通道）
        """
        super().__init__()
        self.in_dim = in_dim
        ...
        self.mixer = DynamicInceptionMixer(in_dim)
        ...
        if downsample_mode == 'conv':
            ...
            self.downsample = nn.Sequential(
                nn.MaxPool2d(kernel_size=2, stride=2),
                nn.Conv2d(in_dim, out_dim, kernel_size=1),  # 通道扩展
                nn.BatchNorm2d(out_dim),
                nn.SiLU()
            )
        else:
        ...
        self.residual_downsample = nn.Sequential(
            ...
            nn.Conv2d(in_dim, out_dim, kernel_size=1),  # 通道扩展: in_dim -> out_dim
            nn.BatchNorm2d(out_dim)
        )

        # 第二部分处理 (使用扩展后的通道维度)
        self.norm2 = nn.BatchNorm2d(out_dim)
        self.mlp = ConvolutionalGLU(out_dim)  # 在扩展后的通道上工作

        # DropPath正则化
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()

        # Layer scale参数 (使用输出维度)
        layer_scale_init_value = 1e-2
        self.layer_scale_1 = nn.Parameter(
    ...
    def forward(self, x):
        """
        输入: (batch_size, in_dim, H, W)
        输出: (batch_size, out_dim, H/2, W/2)
        """
        # 第一部分: 特征提取+下采样+通道扩展
        branch1 = self.norm1(x)
        branch1 = self.mixer(branch1)  # 特征增强 (out: in_dim × H × W)
        branch1 = self.downsample(branch1)  # 下采样+通道扩展 (out: out_dim × H/2 × W/2)

        # 残差连接处理
        residual1 = self.residual_downsample(x)  # (out: out_dim × H/2 × W/2)

        # 第一部分融合
        x = residual1 + self.drop_path(self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * branch1)

        # 第二部分: MLP处理
        branch2 = self.norm2(x)
        branch2 = self.mlp(branch2)  # (out: out_dim × H/2 × W/2)

        # 第二部分融合
        x = x + self.drop_path(self.layer_scale_2.unsqueeze(-1).unsqueeze(-1) * branch2)
